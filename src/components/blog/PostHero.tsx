...
function BackArrowIcon() {
  return (
    <svg aria-hidden className="size-4 shrink-0 rtl:rotate-180" fill="currentColor" viewBox="0 0 16 16">
      <path
        clipRule="evenodd"
...
export function PostHero({ post }: PostHeroProps) {
  ...
  return (
    <header className="mb-12 pb-8">
      ...
      {post.excerpt && (
        <p className="mt-6 max-w-[65ch] text-base leading-7 text-muted-foreground">{post.excerpt}</p>
      )}

      {cover?.url && (
        <div className="relative mt-10 aspect-video overflow-hidden rounded-lg border border-border bg-warm-accent shadow-[0_4px_20px_rgba(42,34,24,0.08)]">
          <Image
            alt={cover.alt}
